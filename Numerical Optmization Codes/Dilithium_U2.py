...
# ============================================================================
# Weight distribution for Kyber B(2)
# ============================================================================
...
set_vars = collections.namedtuple('LWE', ' t0_0 t1_0 z1_0 z2_0 z3_0 o1_0 o2_0 z1_1 z2_1 z3_1 o1_1 o2_1 t0_1 t1_1 th_1 g b1 b2 lmd')
...
n0_0 = lambda x : x.g - 2*n2_0(x) - 2*n1_0(x)
...
n3_1 = lambda x : x.t1_0 + x.o2_0 + x.z3_0
n2_1 = lambda x : x.t0_0 + x.o1_0 + x.z2_0 + x.o2_0              
n1_1 = lambda x : n2_0(x) - 2*(x.t0_0 + x.t1_0) + n1_0(x)/2 - x.o2_0 + x.z1_0 + x.t1_0
n0_1 = lambda x : n1_0(x) - 2*(x.o1_0 + x.o2_0) + n0_0(x) - 2*(x.z1_0 + x.z2_0 + x.z3_0) +2*x.t0_0


### Layer-1 representation count exponent.
R1 =  lambda x : 2*multiH(n2_0(x),[x.t0_0, x.t1_0, x.t0_0, x.t1_0]) + 2*multiH(n1_0(x),[x.o1_0, x.o1_0, x.o2_0, x.o2_0, n1_0(x)/2 - x.o1_0 -x.o2_0]) + multiH(n0_0(x), [x.z1_0, x.z2_0, x.z3_0, x.z3_0, x.z1_0, x.z2_0])
...
domain = lambda x : multiH(x.g,[n3_2(x), n3_2(x), n2_2(x), n2_2(x), n1_2(x), n1_2(x)])+ multiH((1-x.g)/4,[(1-x.g*x.b1)*w1/4,(1-x.g*x.b2)*w2/4,(1-x.g*x.b1)*w1/4,(1-x.g*x.b2)*w2/4])



### Exponent corresponding to the restricted secret domain.
ss = lambda x : multiH(x.g,[n1_0(x) , n1_0(x), n2_0(x), n2_0(x)]) + 4*multiH((1-x.g)/4,[(1-x.g*x.b1)*w1/4,(1-x.g*x.b2)*w2/4,(1-x.g*x.b1)*w1/4,(1-x.g*x.b2)*w2/4])
...
def time(x):
    ...
    good_D = multiH(1,[w1, w1, w2, w2]) - ss(x)
...
constraints = [
    ...
    # Domain restriction
    { 'type' : 'ineq',   'fun' : lwe(lambda x :  multiH(1,[w1, w1, w2, w2]) - ss(x))},

    # Feasibility condition
    { 'type' : 'ineq',   'fun' : lwe(lambda x : (1-x.g)/4 - (1-x.g*x.b1)*w1/2 - (1-x.g*x.b2)*w2/2)},
...
def single_optimization_run(run_id):
    ...
    start = [ru(0, 0.009) for _ in range(num_params-3)] + [ru(0, 0.01) for _ in range(3)] + [ru(0.05,0.5)]

    bounds=[(0.,0.1)]*(num_params-3)+[(.01,1)]*3 + [(0,0.5)]
