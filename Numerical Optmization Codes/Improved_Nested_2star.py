...
constraints = [
    ...
    { 'type' : 'ineq',   'fun' : lwe(lambda x : (1-x.g)/4 - (1-x.g*x.b)*w/4)},
    ...
    { 'type' : 'ineq',   'fun' : lwe(lambda x : n1_0(x)/2- x.o1_0)},
    { 'type' : 'ineq',   'fun' : lwe(lambda x : -x.z1_0 - x.z2_0 + n0_0(x)/2)},
    ...
    { 'type' : 'ineq',   'fun' : lwe(lambda x : n2_1(x)/2- x.t_1)},
    { 'type' : 'ineq',   'fun' : lwe(lambda x : n1_1(x)/2- x.o1_1)},
    { 'type' : 'ineq',   'fun' : lwe(lambda x : -x.z1_1 - x.z2_1 + n0_1(x)/2)},
...
def single_optimization_run(run_id):
    ...
    start = [ru(0, 0.009) for _ in range(7)] + [ru(0, 0.01) for _ in range(2)]+[ru(0, 0.5) for _ in range(1)]

    bounds=[(0.,0.1)]*7+[(.01,1)]*2+[(0,0.5)]
